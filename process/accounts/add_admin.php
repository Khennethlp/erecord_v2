...
if ($method == 'fetch_account') {
    ...
    if ($stmt->rowCount() > 0) {
        foreach ($stmt->fetchAll() as $j) {
            ...
            echo '<tr style="cursor:pointer;" class="modal-trigger" data-toggle="modal" data-target="#update_admin" onclick="get_account_details(&quot;' . $j['id'] . '~!~' . $j['fname'] . '~!~' . $j['username'] . '~!~' . $j['role'] . '~!~' . '&quot;)">';
...
if ($method == 'search_account_list') {
    ...
    $query = "SELECT * FROM accounts WHERE username = '$username_search' AND role = 'admin'";
