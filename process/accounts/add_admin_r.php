...
if ($method == 'fetch_account') {
    ...
    if ($stmt->rowCount() > 0) {
        foreach ($stmt->fetchAll() as $j) {
            ...
            echo '<tr style="cursor:pointer;" class="modal-trigger" data-toggle="modal" data-target="#update_admin_r" onclick="get_account_details(&quot;' . $j['id'] . '~!~' . $j['fname'] . '~!~' . $j['username'] . '~!~' . $j['role'] . '~!~' . '&quot;)">';
